/** Shared connection logic and small, executable examples for the split-view UI. */
import { OpenAIChatLM, adapterFor, access, type ProviderLM } from "lm15/browser";
...
/** Render a standalone single-turn example, never the application's credential. */
export function example(connection: Connection, prompt: string, mode: ExampleMode): string {
  ...
  if (connection.provider === "custom") {
    ...
    if (connection.provider === "anthropic") lines.push(
      "  access: access.withHeaders(access.ANTHROPIC_API, {",
  ...
  if (mode === "request" || mode === "stream") {
    lines.push("", "const request = {", `  model: ${q(connection.model)},`, `  messages: [Message.user(${q(prompt || "Hello!")})],`, "  config: { maxTokens: 400 },", "};");
  }
  ...
  if (mode === "stream") lines.push("", "const controller = new AbortController();", "const result = new ResponseStream(", "  lm.stream(request, { signal: controller.signal }),", "  request,", ");", "", "for await (const text of result) {", "  console.log(text);", "}", "", "// The Stop button calls controller.abort().");
  return lines.join("\n");
}
